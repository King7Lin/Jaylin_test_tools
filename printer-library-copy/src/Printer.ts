...
class PrinterAPI {
  ...
  connectPrinter(
    ...
    return (async () => {
      ...
      try {
        console.log('------------connectPrinter ok11111', printerId, params);
        if (ok) {
          console.log('------------connectPrinter ok', printerId, params);
          // 首次成功连接后，持久化打印机配置，便于后续自动连接
          ...
          await db.savePrinter({
            id: printerId,
            name: `${
              params.name ? params.name : `${params.type}_${printerId}`
            }`.slice(0, 60),
            type: newContent,
            connectionParams: {
          ...
          console.log('------------connectPrinter savePrinter success');
        }
      } catch (error) {
        console.log('------------Printer connectPrinter error', error);
        throw error;
      }
      console.log('------------Printer connectPrinter ok');
