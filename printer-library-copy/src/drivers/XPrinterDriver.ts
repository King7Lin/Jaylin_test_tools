  ...
  async connect(printerId?: string): Promise<boolean> {
    ...
    for (let attempt = 0; attempt <= max; attempt++) {
      try {
        ...
        // Persist printer config into DB for auto-type routing in enqueue/execute
        if (printerId) {
          try {
            ...
            console.log('--------SavePrinter XPrinter');

            await db.savePrinter({
              id: printerId,
              name: `XP_${this.opts.connectType}_${this.opts.address}`.slice(
                0,
                60,
              ),
              type: 'xprinter',
              connectionParams: {
                connectType: newContent,
